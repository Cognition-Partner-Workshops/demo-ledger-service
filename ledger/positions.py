...
@dataclass
class Position:
    """Net holding of one symbol in one account, with its total cost basis."""
    ...
    def average_cost(self) -> Decimal:
        """Cost basis per unit held; zero when the position is flat."""
        if self.quantity == 0:
            return Decimal("0")
...
def build_positions(trades: Iterable[Trade]) -> list[Position]:
    """Aggregate trades into one Position per (account, symbol), sorted by that key.

    Sells reduce the position at its average cost. A sell larger than the
    open position raises ValueError.
    """
...
def gross_exposure(positions: Iterable[Position], prices: Mapping[str, Decimal]) -> Decimal:
    """Sum of absolute position quantities times the given price per symbol."""
    total = Decimal("0")
